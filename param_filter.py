        ...
        with open(self.output_file, "w") as f:
            for each_parameter in self.base.parameter_list:
                if not each_parameter.name or not each_parameter.set or not each_parameter.get:
                    print(f"参数 {each_parameter.name} 配置不完整，跳过")
                    continue

                # 如果参数是范围设置，跳过
                if each_parameter.is_range():
                    print(f"参数 {each_parameter.name} 是范围设置，跳过监测,直接加入。")
                    self.significant_params.append(each_parameter)
                    continue

                print(f"开始测试参数 {each_parameter.name}，当前值: {each_parameter.default}")

                # 遍历 options 中的每个值
                for option in each_parameter.options:
                    if option == each_parameter.default:
                        print(f"参数 {name} 的值已是 {option}，跳过")
                        continue
                    # 修改参数值
                    try:
                        subprocess.run(each_parameter.set.format(option), shell=True, check=True)
                        print(f"已将参数 {each_parameter.name} 修改为 {option}")
                    except subprocess.CalledProcessError as e:
                        print(f"无法设置参数 {each_parameter.name} 的值为 {option}: {e}")
                        continue
                    ...
                    try:
                        subprocess.run(each_parameter.set.format(option), shell=True, check=True)
                        print(f"已将参数 {each_parameter.name} 恢复为原始值 {each_parameter.default}")
                    except subprocess.CalledProcessError as e:
                        print(f"无法恢复参数 {each_parameter.name} 的值为 {each_parameter.default}: {e}")
                        continue
                    # 计算百分比差值
                    ...
                    print(f"记录完成: 参数 {each_parameter.name}, 修改值 {option}, 耗时 {elapsed_time:.2f} 秒, 百分比差值: {percentage_diff:.2f}%, {status}")
                    if percentage_diff > self.base.improvement and each_parameter not in self.significant_params:
                        self.significant_params.append(each_parameter)
        print(f"所有参数测试完成，结果已保存至 {self.output_file}")
